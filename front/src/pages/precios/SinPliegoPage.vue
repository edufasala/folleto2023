      ...
      <q-card>
        ...
        <q-card-section class="q-pa-md">
              <q-form @submit="submit">
                ...
                <q-select v-model="dato.seModifica" label="Se Modifica" outlined dense
                          :options="['si', '']"/>
                <q-select v-model="dato.fracciona" label="Fracciona" outlined dense
                          :options="['1', '0,5', '250', '375', '500', '1000', '-']"/>
                <q-select v-model="dato.espacios" label="Espacios" outlined dense
                          :options="['1', '0,5']"/>
  ...
  methods: {
    ...
    eliminar (dato) {
      this.$q.dialog({
        ...
        this.$axios.delete(`sinPliegos/${dato.id}`).then(() => {
          this.datosGet()
        })
      })
    },
    submit () {
      ...
      if (this.dialogOption === 'create') {
        this.$axios.post('sinPliegos', this.dato).then(() => {
          this.datosGet()
          this.dialog = false
        }).finally(() => {
          this.loading = false
        ...
        this.$axios.put(`sinPliegos/${this.dato.id}`, this.dato).then(() => {
          this.datosGet()
          this.dialog = false
        }).finally(() => {
          this.loading = false
